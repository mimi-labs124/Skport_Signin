...
class ConfigureSitesTests(unittest.TestCase):
    def test_configure_sites_keeps_endfield_only_by_default(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            configure_sites.configure_sites(
                config_path,
                include_arknights=False,
                share_profile_with_arknights=True,
            )

            data = json.loads(config_path.read_text(encoding="utf-8"))

        self.assertEqual([site["key"] for site in data["sites"]], ["endfield"])
        self.assertEqual(data["sites"][0]["browser_profile_dir"], "../state/browser-profile")

    def test_configure_sites_adds_arknights_with_shared_profile(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            config_path = Path(temp_dir) / "settings.json"
