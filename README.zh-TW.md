...
Skport_Signin 是一個非官方、以 Windows 為主的 SKPORT 每日簽到自動化工具，目前支援：
...
Skport_Signin 會把登入 session 保存在本機。請在分享任何工作區或發行包前，先閱讀 [SECURITY.md](./SECURITY.md)。
...
`skport_signin package ...` 只支援原始碼模式。打包後的 `skport_signin.exe` 可執行日常操作命令，但不應拿來重新建 PyInstaller 產物。
...
如果 Skport_Signin 有幫你省下時間，想支持後續維護、測試與打包工作，可以透過 Ko-fi 支持 MimiLab：

[在 Ko-fi 支持 Skport_Signin](https://ko-fi.com/mimilab)

支持完全自願。Skport_Signin 仍然是一個非官方、以個人使用為主的自動化工具。
